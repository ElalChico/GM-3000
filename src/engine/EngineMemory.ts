...
import { loadExternalEnginePayload } from "./EngineTranslator";
...
interface LearnEntry {
  winWeight: number;
  errorWeight: number;
  tdWeight: number; // Nuevo campo para aprendizaje por diferencia temporal
  visits: number;
  lastSeen: number;
}

// Parámetros de aprendizaje - editables manualmente para experimentar
const DECAY_HALF_LIFE_MS  = 30 * 24 * 60 * 60 * 1000; // 30 días (recuerdos duran más)
const MAX_LEARN_ENTRIES   = 15000; // Capacidad aumentada
const MAX_PATTERNS        = 5000;
const SAVE_QUOTA_BYTES    = 4_500_000; 
const DIRTY_SAVE_INTERVAL = 40; // Aumentado de 15 a 40 para evitar tildes por guardado frecuente
...
//         `${this.learnMap.size} posiciones, ${this.recentHistory.length} patrones (${source})`
//       );
    } catch (e) {
    ...
    try {
      if (serialized.length <= SAVE_QUOTA_BYTES) {
        ...
        const trimmed = JSON.stringify({
          ...
          motorName: this.memoryName,
          learnMap: Array.from(this.learnMap.entries()),
          patterns: this.recentHistory.slice(-Math.floor(MAX_PATTERNS * 0.7)),
        });
...
//       console.log(
//         `[EngineMemory:${this.memoryName}] Guardado: ${this.learnMap.size} pos, ${this.recentHistory.length} pat`
//       );
  }
  ...
  public recordMove(
    ...
    if (existing) {
      // Los buenos recuerdos duran más (decay más lento si winWeight es alto)
      ...
      if (result === "win") {
        ...
        // Si es una victoria clara, reducimos el errorWeight previo (reemplazo por mejor jugada)
  ...
  /**
   * Propaga el valor de una posición futura hacia atrás (Temporal Difference Learning).
   * Permite al motor aprender si un movimiento fue "mejor de lo esperado" antes del fin del juego.
   */
  public recordMoveWithTD(
    fen: string,
    move: string,
    immediateScore: number, // Evaluación estática o superficial
    futureValue: number,    // Valor obtenido tras búsqueda profunda
    depth: number
  ): void {
    const key = this.generatePatternKey(fen, move);
    const now = Date.now();
    
    // El error TD es la diferencia entre lo que esperábamos y lo que descubrimos al buscar profundo
    const tdError = futureValue - immediateScore;
    
    // Factor de aprendizaje (learning rate) ajustado por contexto
    const contextWeight = this.getContextWeight(fen);
    ...
    if (existing) {
      ...
      existing.visits++;
      existing.lastSeen = now;
      // Limitar el peso TD para que no desvirtúe totalmente la evaluación base
  ...
  public teachFromEngine(engineName: string, influenceWeight: number = 0.7): boolean {
    const normalizedSource = String(engineName).trim();
    if (!normalizedSource || normalizedSource === this.memoryName) return false;
    const payload = loadExternalEnginePayload(normalizedSource);
    if (!payload || !Array.isArray(payload.learnMap)) return false;

    influenceWeight = Math.max(0.2, Math.min(1.0, influenceWeight));
    let mergedCount = 0;

    for (const entry of payload.learnMap) {
      if (!Array.isArray(entry) || entry.length < 2) continue;
      const [rawKey, rawValue] = entry;
      if (typeof rawKey !== "string") continue;
      const key = this.resolveExternalPatternKey(rawKey, rawValue);
      const external = this.createEntryFromExternal(rawValue, influenceWeight);
      if (!external) continue;

      const existing = this.learnMap.get(key);
      if (existing) {
        existing.winWeight += external.winWeight;
        existing.errorWeight += external.errorWeight;
        existing.tdWeight += external.tdWeight;
        existing.visits += external.visits;
        existing.lastSeen = Math.max(existing.lastSeen, external.lastSeen);
      } else {
        this.learnMap.set(key, external);
      }
      mergedCount++;
    }

    if (Array.isArray(payload.recentHistory)) {
      const historySlice = payload.recentHistory.slice(-100).map((item: any) => ({
        ...item,
        timestamp: Date.now(),
      }));
      this.recentHistory.push(...historySlice);
      if (this.recentHistory.length > MAX_PATTERNS) {
        this.recentHistory.splice(0, this.recentHistory.length - MAX_PATTERNS);
      }
    }
  ...
  private resolveExternalPatternKey(rawKey: string, rawValue: any): string {
    if (rawKey.includes(":")) {
      const [fen, move] = rawKey.split(":", 2);
      if (fen && move) return this.generatePatternKey(fen, move);
    }
    return rawKey;
  }

  private createEntryFromExternal(raw: any, weight: number): LearnEntry | null {
    if (!raw || typeof raw !== "object") return null;
    const winWeight = typeof raw.winWeight === "number" ? raw.winWeight * weight : 0;
    const errorWeight = typeof raw.errorWeight === "number" ? raw.errorWeight * weight : 0;
    const tdWeight = typeof raw.tdWeight === "number"
      ? raw.tdWeight * weight
      : typeof raw.avgDelta === "number"
      ? Math.max(-5, Math.min(5, raw.avgDelta * 0.02 * weight))
      : 0;
    const visits = Math.max(1, Math.round(raw.visits || 1));
  ...
  // --------------------------------------------------------------------------
  // Gestión de Patrones Estratégicos (Planes de Juego)
  // --------------------------------------------------------------------------
  ...
  // --------------------------------------------------------------------------
  // Perfiles de Oponente y Amenazas
  // --------------------------------------------------------------------------
  ...
  public getMoveBonus(fen: string, move: string): number {
    ...
    // Bonus de Exploración (estilo UCB) para descubrir nuevos movimientos buenos
    let explorationBonus = 0;
    if (!entry || entry.visits < 5) {
      // Calculamos un "total de visitas" aproximado para la posición actual (MB_PS_T)
      ...
      explorationBonus = Math.min(explorationBonus, 400); // Límite exacto sugerido
    }

    if (!entry) return Math.round(explorationBonus);

    const decay      = decayFactor(entry.lastSeen);
    const wins       = entry.winWeight; // No decay para victorias (recuerdos duraderos)
    const errors     = entry.errorWeight * decay;
    const tdBonus    = (entry.tdWeight || 0) * 200; // El peso TD se traduce a puntos de evaluación
  ...
  /**
   * Determina si un movimiento debe ser podado inmediatamente basándose en el historial de patrones.
   * Si la confianza de error es muy alta, el motor ahorra tiempo no evaluando esta rama.
   */
  public getPruningAdvice(fen: string, move: string): { prune: boolean, reason: string } {
    ...
    if (entry && entry.visits > 10) {
      ...
      // Si el castigo es extremo (-5000 o menos), recomendamos podar
      if (bonus <= -5000) {
  ...
  public isKnownBadMove(fen: string, move: string): boolean {
    ...
    const decay = decayFactor(entry.lastSeen);
    // Sensibilidad aumentada para detectar movimientos malos rápidamente
  ...
  public getMemoryStats() {
    ...
    return {
      totalPatterns:    this.recentHistory.length,
...
//     console.log(`[EngineMemory:${this.memoryName}] Memoria reiniciada.`);
  ...
  /**
   * Genera una clave de características que agrupa posiciones similares.
   * Esto permite al motor "generalizar" lo aprendido en lugar de solo
   * recordar FENs exactos.
   */
  private generatePatternKey(fen: string, move: string): string {
    const parts = fen.split(" ");
    const boardStr = parts[0]; // Piezas
    const turn = parts[1];    // Turno
    
    // 1. Balance de material (simplificado)
    const materialBalance = this.calculateTotalMaterial(boardStr, true);

    // 2. Patrón de estructura de peones (solo filas y columnas donde hay peones)
    const pawnStructure = boardStr.replace(/[^pP/]/g, ".");

    // 3. Seguridad básica (si hay piezas cerca del rey - muy simplificado)
    // Usamos el FEN normalizado como base pero le añadimos las características
    const fenKey = this.normalizeFen(fen);
    
    // Combinamos: Material_Estructura_Turno_Movimiento
    // Esto hace que si la estructura de peones y el material son iguales, 
    // el motor pueda "intuir" que el movimiento puede ser similarmente bueno/malo.
  ...
  private getContextWeight(fen: string): number {
    ...
    if (material > 2600) return 0.7; // Apertura: aprender más de teoría (sugerido 0.7)
    if (material > 1300) return 1.0; // Medio juego: aprendizaje estándar
    return 1.3; // Final: cada error cuenta más (sugerido 1.3)
  }
...
//     console.log(`[EngineMemory:${this.memoryName}] Poda realizada: ${removeCount} posiciones irrelevantes eliminadas.`);
  ...
  private aggressivePrune(fraction: number): void {
    ...
    this.recentHistory = this.recentHistory.slice(-Math.floor(MAX_PATTERNS * (1 - fraction)));
  }
